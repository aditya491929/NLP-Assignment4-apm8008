...
def do_train(args, model, train_dataloader, save_dir="./out"):
    ...
    for epoch in range(num_epochs):
        for batch in train_dataloader:
            ...
            # Forward pass with labels -> model returns loss
            outputs = model(**batch)
            loss = outputs.loss
...
def create_augmented_dataloader(args, dataset):
    ...
    # 1) Take the original training split (raw, with "text" and "label")
    train_raw = dataset["train"]
    print("Original train size:", len(train_raw))

    # 2) Sample 5k random examples from the training set
    num_aug = 5000
    sampled_raw = train_raw.shuffle(seed=42).select(range(num_aug))

    # 3) Apply our custom transformation from utils.py to these 5k examples
    #    load_from_cache_file=False so that the random transformations are actually applied
    transformed_raw = sampled_raw.map(
        custom_transform,
        load_from_cache_file=False
    )
    print("Transformed sample size:", len(transformed_raw))

    # 4) Concatenate original + transformed training examples
    #    (same label, but text is now corrupted / paraphrased)
    augmented_raw = datasets.concatenate_datasets([train_raw, transformed_raw])
    print("Augmented train size:", len(augmented_raw)) 

    # 5) Tokenize augmented dataset with the same tokenizer & function
    ...
    # 6) Prepare for PyTorch: drop "text", rename "label" -> "labels", set format
